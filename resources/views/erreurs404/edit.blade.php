<!-- resources/views/erreurs404/edit.blade.php -->
@extends('layouts.app')
@vite(['resources/css/app.css', 'resources/js/app.js'])

@section('content')
    <div class="container max-w-4xl mx-auto p-6 bg-white rounded-xl shadow-xl mt-10">
        <h1 class="text-3xl font-bold text-gray-900 mb-8">Modifier l'Erreur 404</h1>
        ...
        <form action="{{ route('erreurs404.update', $erreur->id_404) }}" method="POST" class="space-y-6">
            @csrf
            @method('PUT')
            ...
            <div>
                ...
                <input type="number" 
                       name="nb_404" 
                       id="nb_404" 
                       value="{{ old('nb_404', $erreur->nb_404) }}"
            ...
            <div>
                ...
                <input type="text" 
                       name="file_data" 
                       id="file_data" 
                       value="{{ old('file_data', $erreur->file_data) }}"
                       class="w-full px-4 py-3 rounded-lg border border-gray-300 focus:outline-none focus:ring-2 focus:ring-blue-500 shadow-sm"
                       maxlength="255" 
                       required>
                @error('file_data')
                ...
                <select name="id_rapport" 
                    ...
                    @foreach($rapports as $rapport)
                        <option value="{{ $rapport->id_rapport }}" 
                                {{ old('id_rapport', $erreur->id_rapport) == $rapport->id_rapport ? 'selected' : '' }}>
